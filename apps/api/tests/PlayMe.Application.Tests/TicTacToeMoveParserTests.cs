...
/// <summary>
/// Unit tests for the unified <see cref="TicTacToeMoveParser"/> (Sprint 9
/// PR1b). The platform calls this with an opaque <see cref="MoveDto.Payload"/>
/// and the parser is the only place the <c>{"cell": int}</c> shape lives on
/// the API side (CLAUDE.md §7 "Platform thinness").
/// </summary>
public sealed class TicTacToeMoveParserTests
{
    ...
    [Fact]
    public void GameId_is_tictactoe()
    {
        _parser.GameId.Value.Should().Be("tictactoe");
    }
    ...
    public void Parse_accepts_valid_cell()
    {
        ...
        result.Value.Should().BeOfType<TicTacToeMove>().Which.Cell.Should().Be(17);
    }
    ...
    public void Parse_rejects_missing_cell()
    {
        ...
        result.Error.Should().Be(TicTacToeErrors.ValidationMove);
    }
