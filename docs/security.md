...
- **API:** every controller action and SignalR Hub method validates its DTO before reaching a handler — no `[FromBody]` payload reaches `Application` unchecked. The seam is **handler-internal** (e.g. `DisplayName.Create`, `RoomCode` ctor, `Enum.IsDefined` on `SideSelectionMode`, the per-game `IGameMoveParser.Parse`): each handler runs its own checks and returns typed `PlatformErrors` keys that are already i18n keys, so failures flow back to the client through the existing `AppResult<T> → ProblemDetails` pipeline. There is **no FluentValidation dependency** — MVC auto-validation produces `ValidationProblemDetails` whose `.errors` map doesn't carry i18n keys, and a mapping layer for failure shapes the handlers already cover wouldn't earn its complexity. The handler-internal checks above are the whole validation story.
...
| `RoomHub.SubmitSetup` | Application | per session | 5 / 10 s (one-shot per side per match; the limit just absorbs commit-button double-clicks) |
| `RoomHub.OfferRematch` / `Accept` / `Reject` | Application | per session | 5 / 10 s (shared policy across the three handshake calls) |
| `RoomHub.Resign` | Application | per session | 3 / 10 s |
| `RoomHub.ExitRoom` | Application | per session | 3 / 10 s |
...
- **Content-Security-Policy** — strict, and different per surface. **Web:** `default-src 'self'` with a nonce-based `script-src` in production and an explicit allowlist (Sentry, PostHog) in `connect-src`; no unnonced inline scripts. **API:** `default-src 'none'; frame-ancestors 'none'` — it never serves HTML, so nothing is allowlisted.
