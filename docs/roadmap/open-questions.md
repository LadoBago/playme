...
- **Receiver-side emote mute.** In-match emotes ([`platform.md`](../platform.md) §3) ship in v1 with a strict per-session server rate limit as the only abuse control. A per-match client toggle to suppress *incoming* emotes was deliberately deferred — revisit if the rate limit proves insufficient in practice (a persisted preference + a small toggle in the match UI).
