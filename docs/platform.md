...
3. **Chess clock** — fixed total per player, server-authoritative, ticks server-side. **Three presets only** (no custom values, no "unlimited" in v1): **1 min**, **3 min**, **10 min** per player. Both players always get the same time bank. Modeled as a strongly-typed enum `TimeLimit { OneMin, ThreeMin, TenMin }` on the API; the handler rejects anything else with `errors.config.invalidTimeLimit` (handler-internal validation per [`security.md`](security.md) §3). Room state stores the resolved `timeLimitMs` for the clock model in [`state.md`](state.md) §2. **Per-game budgets** come from each module's `IGameModule.ClockBudgetFor(gameOptions)`: the platform hands the module the room's opaque options blob and reads back a budget, never inspecting the blob itself (CLAUDE.md §7 "Platform thinness"). `connect4` → 3 min, `reversi` → 10 min, `seabattle` → 10 min (battle phase). **`tictactoe` scales its budget by board size**: 3×3 → 1 min, 6×6 → 3 min, 9×9 → 5 min. The host-selected time-limit picker (the `TimeLimit` enum / `timeLimitMs` above) is not built yet; until it lands, the module-derived budget is authoritative and these presets describe the intended future override (see [`roadmap/deferred-polish.md`](roadmap/deferred-polish.md)).
4. **Online move pipeline** — every move travels client → server → opponent via SignalR. **Turn order is module-decided per accepted move** (Sprint 10 seam B): by default an accepted move flips the turn to the other side, but a module may retain the mover's turn via `MoveResult.KeepTurn` (sea battle's hit-shoots-again). The platform does not guarantee strict alternation; it guarantees that only the side-to-move can act and that the chess clock charges whoever is on the move — including across retained turns. `KeepTurn` is the single mechanism for module-decided turn retention — both frequent earned extra turns (sea battle's hit) and rare forced skips (Reversi stranding the opponent). The Sprint 8 renderer-emitted synthetic-move pattern is retired (June 2026 — see [`roadmap/sprint-08-reversi.md`](roadmap/sprint-08-reversi.md)).
...
12. **Clock-start rule** — the clock for the side that moves first **starts immediately when the match enters `InProgress`**. For setup-less games that is the moment both players are present in the room — no "ready up" step. For setup games (Sprint 10 seam C) it is the moment both setups are committed; the setup phase itself is **unclocked** and bounded by the module's `SetupBudget` instead (see [`state.md`](state.md) §2.1 `SettingUp`).
...
**Optional capabilities.** Beyond `IGameModule`, a module may opt into platform capabilities by implementing additional interfaces; the platform dispatches by capability check, never by `GameId`. Currently:

- `IHiddenStateGame` (Sprint 10 seam A) — per-viewer wire projection for games with hidden information; persistence keeps the full state, projection applies only while the match has no outcome, and a `null` viewer side selects the module's public view (see [`security.md`](security.md) §4 and [`state.md`](state.md) §2.3 for the delivery rules).
- `ISetupGame` (Sprint 10 seam C) — a pre-match, simultaneous, secret setup phase. Rooms enter `SettingUp` instead of `InProgress` once both players are present; each side commits one final setup payload (`SubmitSetup`), the module decides completeness (`IsSetupComplete` — asymmetric setup stays module territory), and the phase is unclocked, bounded by the module's `SetupBudget` (see [`state.md`](state.md) §2.1 and #12 above). One commit per side is platform-enforced (`errors.setup.alreadyCommitted`); payload shape and validation vocabulary stay module-owned. Rematches re-enter setup with fresh state.
...
- **Emote reactions.** Either player can send an ephemeral emote (the fixed set `smile` / `like` / `heart` / `clap` / `poke` / `cry`) to the opponent during active play **and** on the post-match / rematch screen. It is a **platform capability, not game state** — it works for every module with zero per-game code (`SendEmote` Hub method, [`architecture.md`](architecture.md) §3.3). The sender picks from the allowlist in a controls-band picker; the opponent sees a **transient, auto-dismissed bubble** anchored to their clock face. Blocked during `SettingUp` and `WaitingForOpponent`. Server-validated against the allowlist and **strictly rate-limited per session** ([`security.md`](security.md) §5); over-limit or out-of-phase sends are dropped silently. Carries **no room state** — never persisted, never replayed on reconnect (`EmoteReceived`, [`state.md`](state.md) §2.3). Receiver-side mute is deferred to v2 ([`roadmap/open-questions.md`](roadmap/open-questions.md)).
